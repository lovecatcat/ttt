      ...
      <template v-if="item.child">
        <div class="am-list-body" v-for="childitem in item.child">
          ...
          <div class="am-list-item" v-if="clientvalue.app_fields[childitem.ci_id]">
            <div class="am-ft-md qaline"><span class="am-ft-red" v-html="clientvalue.app_fields[childitem.ci_id]"></span>
            </div>
          </div>
          <app-form :ref="'appform'+childitem.ci_id"
      ...
      <template v-else>
        <div class="am-list-body">
          ...
          <div class="am-list-item" v-if="clientvalue.app_fields[item.ci_id]">
            <div class="am-ft-md qaline"><span class="am-ft-red" v-html="clientvalue.app_fields[item.ci_id]"></span>
            </div>
          </div>
          <app-form :ref="'appform'+item.ci_id"
                    :k="'appl'+item.ci_id"
                    v-if="appForms[item.entry]"
                    :forms="appForms[item.entry]"
                    :index="item.ci_id"
        ...
        <div class="app-pupover" v-show="show">
          <div class="am-list form">
            <template v-for="item in forms">
              <template v-show="index=='189'&&item.ci_id=='189'" v-if="index=='189'&&item.ci_id=='189'">
              ...
              <template v-show="index!='189'" v-if="index!='189'&&item.ci_id!='189'">
        ...
        methods: {
          ...
          checkForm () {
            ...
            for (let i in this.forms) {
              let j = this.forms[i]
              if (this.index === '189' && !this.forms[0].input) {
                this.$toast.open('请填写' + j.title, '')
                bool = false
                break
              } else if ((this.index === '190' || this.index === '191') && i > 0 && !j.input) {
                this.$toast.open('请填写' + j.title, '')
                bool = false
                break
              } else if (['189', '190', '191'].indexOf(this.index) === -1 && !j.input && j.type !== 'radio') {
                this.$toast.open('请填写' + j.title, '')
                bool = false
                break
              }
...
//                j.input = ''
              ...
              if (this.index === '190' || this.index === '191') {
                if (i > 0) {
                  field += j.title + '：' + text + '\t'
                }
              } else if (this.index !== '189') {
                field += j.title + '：' + text + '\t'
              }
            }
            if (this.index === '189') {
    ...
    created () {
      ...
      Api.queryMatters(res => {
        ...
        vm.matters = res.filter(matter => {
          return matter.version === '1'
        })
        vm.$store.dispatch('saveMatters', Api.obj2json(vm.matters))
      })
      this.clientvalue.ass_amswer = Api.obj2json(this.$store.state.clientvalue).ass_amswer
      this.clientvalue.fields = Api.obj2json(this.$store.state.clientvalue).fields
    ...
    methods: {
      noNeedChoose(id) {
        ...
        if (age >= 2 && (['193', '194'].indexOf(id) !== -1)) {
          toast_text = '投保人小于2周岁才需填写此项'
        } else if ((['189', '190', '191'].indexOf(id) !== -1) && (age <= 12 || Number(sex) === 11338)) {
          toast_text = '大于12周岁女性才需填写此项'
        } else if (age >= 18 && id === '202') {
